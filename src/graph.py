...
class EdgeList(list):
	def filter(self, **edge_properties):
		if not edge_properties:
			return self
		result = EdgeList()
		for element in self:
			for key, value in edge_properties.items():
				if key in element._properties:
					if element._properties[key] == value:
...
class Node(object):
	...
	def __init__(self, graph, **kwargs):
		if not isinstance(graph, Graph):
			raise GraphError('No Graph supplied.')
		object_setattr = object.__setattr__
		object_setattr(self, '_graph', graph)
		object_setattr(self, '_id', Node._current_id)
		Node._current_id += 1
		object_setattr(self, '_properties', kwargs)
		# maybe change _incoming_edges/_outgoing_edges to sets
		object_setattr(self, '_incoming_edges', [])
		object_setattr(self, '_outgoing_edges', [])
		graph._nodes[self._id] = self
	
	def incoming_edges(self, **edge_properties):
		return EdgeList(self._incoming_edges).filter(**edge_properties)
	def outgoing_edges(self, **edge_properties):
		return EdgeList(self._outgoing_edges).filter(**edge_properties)
	def edges(self, **edge_properties):
		return EdgeList(self._outgoing_edges + self._incoming_edges).filter(**edge_properties)
	
	def incoming_nodes(self, **node_properties):
		pass
	def outgoing_nodes(self, **node_properties):
		pass
	def nodes(self, **node_properties):
		pass
...
class Edge(object):
	...
	def __init__(self, head, label, tail, **kwargs):
		if not isinstance(head, Node):
			raise GraphError('head is not a Node')
		if not isinstance(tail, Node):
			raise GraphError('tail is not a Node')
		if head._graph != tail._graph:
			raise GraphError('Graph of head is not same as tail.')
		if not isinstance(head._graph, Graph):
			raise GraphError('Graph of head or tail is None.')
		object_setattr = object.__setattr__
		object_setattr(self, '_graph', head._graph)
		object_setattr(self, '_id', Edge._current_id)
		object_setattr(self, 'label', label)
		Edge._current_id += 1
		object_setattr(self, '_properties', kwargs)
		object_setattr(self, 'head', head)
		object_setattr(self, 'tail', tail)
		self.head._outgoing_edges.append(self)
		self.tail._incoming_edges.append(self)
		self._graph._edges[self._id] = self
